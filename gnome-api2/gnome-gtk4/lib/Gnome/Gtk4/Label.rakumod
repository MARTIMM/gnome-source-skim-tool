...
my Hash $methods = %(
  ...
  get-selectable => %( :returns(gboolean)),
  get-selection-bounds => %( :returns(gboolean), :parameters([gint-ptr, gint-ptr])),
  get-single-line-mode => %( :returns(gboolean)),
  get-text => %( :returns(Str)),
  get-use-markup => %( :returns(gboolean)),
  get-use-underline => %( :returns(gboolean)),
  get-width-chars => %( :returns(gint)),
  get-wrap => %( :returns(gboolean)),
