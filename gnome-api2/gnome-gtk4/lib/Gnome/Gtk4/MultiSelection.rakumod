...
unit class Gnome::Gtk4::MultiSelection:auth<github:MARTIMM>:api<2>;
...
#`{{
  ...
  if self.^name eq 'Gnome::Gtk4::MultiSelection' {
    # If already initialized using ':$native-object', ':$build-id', or
    # any '.new*()' constructor, the object is valid.
    note "Native object not defined, .is-valid() will return False" if $Gnome::N::x-debug and !self.is-valid;

    # only after creating the native-object, the gtype is known
    self._set-class-info('GtkMultiSelection');
  }
}
...
my Hash $methods = %(
  ...
  #--[Constructors]-------------------------------------------------------------
  new-multiselection => %( :type(Constructor), :is-symbol<gtk_multi_selection_new>, :returns(N-Object), :parameters([ N-Object]), ),

  #--[Methods]------------------------------------------------------------------
  get-model => %(:is-symbol<gtk_multi_selection_get_model>, :returns(N-Object), ),
  set-model => %(:is-symbol<gtk_multi_selection_set_model>, :parameters([N-Object]), ),
);
