...
submethod BUILD ( *%options ) {
  ...
  unless $signals-added {
    self.add-signal-types( $?CLASS.^name,
      :w0<select-all unselect-all activate-cursor-child selected-children-changed toggle-cursor-child>,
      :w1<child-activated>,
      :w4<move-cursor>,
    );
    $signals-added = True;
...
my Hash $methods = %(
  ...
  #bind-model => %(:is-symbol<gtk_flow_box_bind_model>,  :parameters([N-Object, :( gpointer $item, gpointer $user-data --> N-Object ), gpointer, ])),
  ...
  #set-filter-func => %(:is-symbol<gtk_flow_box_set_filter_func>,  :parameters([:( N-Object $child, gpointer $user-data --> gboolean ), gpointer, ])),
  ...
  #set-sort-func => %(:is-symbol<gtk_flow_box_set_sort_func>,  :parameters([:( N-Object $child1, N-Object $child2, gpointer $user-data --> gint ), gpointer, ])),
