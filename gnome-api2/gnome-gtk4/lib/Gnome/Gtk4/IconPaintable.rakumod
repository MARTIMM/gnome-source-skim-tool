...
unit class Gnome::Gtk4::IconPaintable:auth<github:MARTIMM>:api<2>;
also is Gnome::GObject::Object;
also does Gnome::Gtk4::R-SymbolicPaintable;
...
submethod BUILD ( *%options ) {
  ...
  unless $signals-added {
    ...
    self._add_gtk_symbolic_paintable_signal_types($?CLASS.^name)
      if self.^can('_add_gtk_symbolic_paintable_signal_types');
    $signals-added = True;
  }
  ...
  if self.^name eq 'Gnome::Gtk4::IconPaintable' {
    # If already initialized using ':$native-object', ':$build-id', or
    # any '.new*()' constructor, the object is valid.
    note "Native object not defined, .is-valid() will return False" if $Gnome::N::x-debug and !self.is-valid;

    # only after creating the native-object, the gtype is known
    self._set-class-info('GtkIconPaintable');
  }
}
...
method _fallback-v2 (
  ...
  if $methods{$name}:exists {
    ...
    my $r;
    my $native-object = self.get-native-object-no-reffing;
    $r = self._do_gtk_symbolic_paintable_fallback-v2(
      $name, $_fallback-v2-ok, $!routine-caller, @arguments, $native-object
    ) if self.^can('_do_gtk_symbolic_paintable_fallback-v2');
