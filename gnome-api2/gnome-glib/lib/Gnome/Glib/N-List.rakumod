...
class N-List:auth<github:MARTIMM>:api<2> is export is repr('CStruct') {
  ...
  submethod BUILD (
    gpointer :$!data, N-List :$!next, N-List :$!prev, 
  ) {
  }
...
my Hash $methods = %(
  ...
  copy-deep => %( :type(Function), :is-symbol<g_list_copy_deep>,  :returns(N-List), :parameters([ N-List, :( gpointer $src, gpointer $user-data --> gpointer ), gpointer])),
  ...
  free1 => %( :type(Function), :is-symbol<g_list_free1>,  :parameters([N-List])),
